...
class UploadExcel extends CI_Model
{
    ...
    public function UploadExcelFile($data, $table_name)
    {
        if ($table_name == 'LUMPSUM') {
            $lum_arr = array('lum_date', 'lum_month', 'lum_s_no', 'lum_of_on', 'lum_client_name', 'lum_fund', 'lum_scheme', 'lum_submit', 'lum_pan_no', 'lum_folio_no', 'lum_amount', 'lum_remark', 'lum_done_ok', 'lum_rm_name', 'lum_client_type', 'lum_category', 'lum_rejection', 'lum_after_liquid_fund');
            ...
            $swp_arr = array( `swp_date`, `swp_s_no`, `swp_month`, `swp_of_on`, `swp_client_name`, `swp_submit`, `swp_scheme_name`, `swp_amount`, `swp_folio_no`, `swp_done_ok`, `swp_date_start`, `swp_date_end`, `swp_duration`, `swp_no_head`);
            $arr  = replace_key($data, $swp_arr);
            $q =  $this->db->insert_batch($table_name, $arr);
        }elseif($table_name == 'STP'){
            $stp_arr = array(`stp_date`, `stp_s_no`, `stp_month`, `stp_of_on`, `stp_client_name`, `stp_amc_name`, `stp_l_e`, `stp_scheme`, `stp_submit`, `stp_to_scheme`, `stp_folio_no`, `stp_amount`, `stp_date_1`, `stp_date_2`, `stp_done_ok`, `stp_remark`, `stp_rm_name`, `stp_comp_continue`, `stp_stp_type`);
